...
Write-Output "$($outObject.returnResult), $ComputerName, $formattedLastLogonDate, $operatingsystem, $Description"

}

$adComputers = Get-ADComputer -Filter "OperatingSystem -Like '*Windows 10*' -and Enabled -eq 'True' " -Properties Name, Description, LastLogonDate, OperatingSystem |Select-Object Name, LastLogonDate, OperatingSystem, Description

$tempstring = "aktualisierbar, name, letztes Login, betriebssystem, beschreibung`n"
...
try {
        $tempstring += Invoke-Command -ComputerName $_.Name -ScriptBlock $hardwarereadiness -ArgumentList $_.Name, $_.LastLogonDate, $_.OperatingSystem, $_.Description  -ErrorAction Stop
        $tempstring += "`n"
}
catch [System.Management.Automation.Remoting.PSRemotingTransportException] {
    # Specific error handling for remote connection issues
    Write-Host "$computerName fehlgeschlagen. Check network or firewall settings." -ForegroundColor Red
}
catch [System.Management.Automation.RemoteException] {
    # Specific error handling for script execution errors on the remote server
    Write-Host "$computerName fehlgeschlagen. The script block failed to execute ." -ForegroundColor Red
}
catch {
    # General error handling for other types of exceptions
    Write-Host "$computerName fehlgeschlagen. An unexpected error occurred on $computerName." -ForegroundColor Red
}
    }
    else {
            $tempstring += "$computername ist nicht online`n"
    }
    write-Host $computerName "abgeschlossen"
}
